...
# markovmodel_R=hmm.GaussianHMM(n_components=2,n_iter=1000,covariance_type="full")
...
for i in range(0,1):
    if (input() != "1"):
        ...
        # directorio_destino = r'C:\Users\sebas\Desktop\Enzimas\Datos_simulados\Gaussiano_3_estados'
        ...
        # ax.legend(loc = 'upper left')
        # ax.set_xlabel("Tiempo")
        # #plt.savefig('his_total_Gaussian_acep_'+str(m1)+'.0_don_'+str(m2)+'.0.png', bbox_inches='tight', transparent=True)
        # plt.show()
        ...
        # #plt.savefig('viterbi_4_states_Gaussian_acep_'+str(m1)+'.0_don_'+str(m2)+'.0.png', bbox_inches='tight', transparent=True)
        # plt.show()
        ...
        # ax.set_ylabel('Estado')
        # ax.set_xlabel('Tiempo')
        # #plt.savefig('viterbi_5_states_Gaussian_acep_'+str(m1)+'.0_don_'+str(m2)+'.0.png', bbox_inches='tight', transparent=True)
        # plt.show()
        ...
        for h in range(4):  # Iterar sobre las cuatro gráficas
            ...
            markovmodel=hmm.GaussianHMM(n_components=3, covariance_type='full',n_iter=1000)
            markovmodel.fit(X)
            ...
            if h == 0:
                res_acep = gaussian_HMM_pseudo_residuals(x=X[:, 0], delta=delta_, gamma=gamma_, mu=lamb_[:,0], sigma=std_matrix[:,0], m=3)
                ...
                res_dono = gaussian_HMM_pseudo_residuals(x=X[:, 1], delta=delta_, gamma=gamma_, mu=lamb_[:,1], sigma=std_matrix[:,1], m=3)
                x = np.arange(-4, 4, 0.001)
                intervalos = range(-6, 7)
                ax.plot(x, norm.pdf(x, 0, 1))
        ...
        for n_comp in range(2,11):
          model = hmm.GaussianHMM(n_components=n_comp,n_iter=1000, covariance_type='full')
          model.fit(X)
