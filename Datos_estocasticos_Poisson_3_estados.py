...
# markovmodel_P= PoissonHMM(n_components=3, n_iter=1000)
...
for i in range(0,1):
    if (input() != "1"):
        ...
        # directorio_destino = r'C:\Users\sebas\Desktop\Enzimas\Datos_simulados\Poisson_3_estados'
        ...
        # # ax.legend(loc = 'upper left')
        # # ax.set_xlabel("Tiempo")
        # # #plt.savefig('his_total_Poisson_acep_'+str(m1)+'_don__'+str(m2)+'.png', bbox_inches='tight', transparent=True)
        # # plt.show()
        ...
        #plt.savefig('viterbi_4_states_Poisson_acep_'+str(m1)+'_don__'+str(m2)+'.png', bbox_inches='tight', transparent=True)
        plt.show()
        ...
        # # ax.set_ylabel('Estado')
        # # ax.set_xlabel('Tiempo')
        # # #plt.savefig('viterbi_5_states_Poisson_acep_'+str(m1)+'_don__'+str(m2)+'.png', bbox_inches='tight', transparent=True)
        # # plt.show()
        ...
        for h in range(4):  # Iterar sobre las cuatro gráficas
            ...
            markovmodel = PoissonHMM(n_components=3, n_iter=1000)
            markovmodel.fit(X)
            ...
            if h == 0:
                res_acep = pois_HMM_pseudo_residuals(x=X[:, 0], delta=delta_, gamma=gamma_, lamb=lamb_[:, 0], m=3)
                ...
                res_dono = pois_HMM_pseudo_residuals(x=X[:, 1], delta=delta_, gamma=gamma_, lamb=lamb_[:, 1], m=3)
                x = np.arange(-4, 4, 0.001)
                intervalos = range(-6, 7)
                ax.plot(x, norm.pdf(x, 0, 1))
        ...
        for n_comp in range(2,11):
          model = PoissonHMM(n_components=n_comp,n_iter=1000)
          model.fit(X)
          print(model.score(X))
