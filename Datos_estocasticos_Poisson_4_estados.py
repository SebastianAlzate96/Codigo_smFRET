...
"""
----------------------------------------------------------------------------
             Datos_estocasticos_Poisson_4_estados
----------------------------------------------------------------------------
...
# transmat = np.array([[0.85,0.05,0.05,0.05],
#                       [0.30,0.35,0.20,0.15],
#                       [0.35,0.05,0.40,0.20],
#                       [0.40,0.03,0.07,0.50]])
...
np.random.seed(1)
gen_model = PoissonHMM(n_components=n_states,n_iter=10000)
...
"""
----------------------------------------------------------------------------
             #  Valores estimados modelo Poisson HMM#
----------------------------------------------------------------------------  
"""

markovmodel_P = PoissonHMM(n_components=n_states,n_iter=10000)
...
# markovmodel_P= PoissonHMM(n_components=4, n_iter=1000)
...
for i in range(0,1):
    if (input() != "1"):
        ...
        # directorio_destino = r'C:\Users\sebas\Desktop\Enzimas\Datos_simulados\Poisson_4_estados'
        ...
        # ruta_destino = os.path.join(directorio_destino, 'his_acep_'+str(m1)+'.0_don_'+str(m2)+'.0.pdf')
        ...
        # ax.legend(loc = 'upper left')
        # ax.set_xlabel("Tiempo")
        # #plt.savefig('his_total_Poisson_acep_'+str(m1)+'.0_don_'+str(m2)+'.0.png', bbox_inches='tight', transparent=True)
        # plt.show()
        ...
        # # Guardar la figura en formato PDF en el directorio especificado
        # # ruta_destino = os.path.join(directorio_destino, 'viterbi_3_states_acep_'+str(m1)+'.0_don_'+str(m2)+'.0.pdf')
        # # plt.savefig(ruta_destino, format='pdf', bbox_inches='tight', transparent=True)
        # plt.show()   
        ...
        ax.set_xlabel('Tiempo')
        plt.yticks(range(0,4,1))
        #plt.savefig('viterbi_4_states_Poisson_acep_'+str(m1)+'.0_don_'+str(m2)+'.0.png', bbox_inches='tight', transparent=True)
        plt.show()
        ...
        # ax.set_ylabel('Estado')
        # ax.set_xlabel('Tiempo')
        # #plt.savefig('viterbi_5_states_Poisson_acep_'+str(m1)+'.0_don_'+str(m2)+'.0.png', bbox_inches='tight', transparent=True)
        # plt.show()
        ...
        for h in range(4):  # Iterar sobre las cuatro gráficas
            ...
            markovmodel = PoissonHMM(n_components=4, n_iter=1000)
            markovmodel.fit(X)
            ...
            if h == 0:
                res_acep = pois_HMM_pseudo_residuals(x=X[:, 0], delta=delta_, gamma=gamma_, lamb=lamb_[:, 0], m=4)
                ...
                res_dono = pois_HMM_pseudo_residuals(x=X[:, 1], delta=delta_, gamma=gamma_, lamb=lamb_[:, 1], m=4)
                x = np.arange(-4, 4, 0.001)
                intervalos = range(-6, 7)
                ax.plot(x, norm.pdf(x, 0, 1))
        ...
        plt.tight_layout()
        # Guardar la figura en formato PDF en el directorio especificado
        # ruta_destino = os.path.join(directorio_destino, 'Pseudo_residuales_acep_'+str(m1)+'.0_don_'+str(m2)+'.0.pdf')
        # plt.savefig(ruta_destino, format='pdf', bbox_inches='tight', transparent=True)
        plt.show()
        ...
        for n_comp in range(2,11):
          model = PoissonHMM(n_components=n_comp,n_iter=1000,random_state=1)
          model.fit(X)
          print(model.score(X))
        ...
        # Guardar la figura en formato PDF en el directorio especificado
        # ruta_destino = os.path.join(directorio_destino, 'BIC_acep_'+str(m1)+'.0_don_'+str(m2)+'.0.pdf')
        # plt.savefig(ruta_destino, format='pdf', bbox_inches='tight', transparent=True)
        plt.show()
