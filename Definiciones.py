...
def pois_HMM_pseudo_residuals(x, delta, gamma, lamb, m):
    ...
    cumdists = np.vstack((np.zeros(n), np.cumsum(cdists, 0), np.full(n, np.nan)))
    ulo = np.full(n, np.nan)
    uhi = np.full(n, np.nan)
    for i in range(n):
...
def gaussian_HMM_lforward(x, gamma, delta, mu, sigma, m):
    n = len(x)
    delta = np.array(delta)
    mu = np.array(mu)
    sigma = np.array(sigma)
    lalpha = np.empty([m, n])
    foo = delta * norm.pdf(x[0], loc=mu, scale=sigma)
    sumfoo = sum(foo)
    lscale = np.log(sumfoo)
    foo = foo / sumfoo
    lalpha[:, 0] = lscale + np.log(foo)
    for i in range(1, n):
        foo = np.dot(foo, gamma * norm.pdf(x[i], loc=mu, scale=sigma))
        sumfoo = sum(foo)
        lscale = np.log(sumfoo)
        foo = foo / sumfoo
        lalpha[:, i] = lscale + np.log(foo)
...
def gaussian_HMM_lbackward(x, gamma, mu, sigma, m):
    n = len(x)
    mu = np.array(mu)
    sigma = np.array(sigma)
    lbeta = np.empty([m, n])
    ...
    lscale = np.log(m)
    for i in np.arange(n-2, -1, -1):
        foo = np.dot(gamma, norm.pdf(x[i+1], loc=mu, scale=sigma) * foo)
        lbeta[:, i] = np.log(foo) + lscale
        sumfoo_ = sum(foo)
        foo = foo / sumfoo_
        lscale = lscale + np.log(sumfoo_)
...
def gaussian_HMM_conditional(xc, x, delta, gamma, mu, sigma, m):
    n = len(x)
    nxc = len(xc)
    dxc = np.empty([nxc, n])
    Px = np.empty([m, nxc])
    for j in range(nxc):
        Px[:, j] = norm.pdf(xc[j], loc=mu, scale=sigma)
    la = gaussian_HMM_lforward(x, gamma, delta, mu, sigma, m)
    lb = gaussian_HMM_lbackward(x, gamma, mu, sigma, m)
    b = np.empty([m, 1])
    b[:, 0] = np.log(delta)
    la = np.hstack((b, la))
    lafact = np.amax(la, 0)
...
def gaussian_HMM_pseudo_residuals(x, delta, gamma, mu, sigma, m):
    x = x.astype(int)
    n = len(x)
    cdists = gaussian_HMM_conditional(np.arange(max(x) + 1), x, delta, gamma, mu, sigma, m)
